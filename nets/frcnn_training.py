...
def bbox_iou(bbox_a, bbox_b):
    if bbox_a.shape[1] != 4 or bbox_b.shape[1] != 4:#数据维数尺寸，第二个数据是不是4
        print(bbox_a, bbox_b)
        raise IndexError
...
#-----ROIpooling层网络的训练，即将上一步建议框当做先验框，则需要计算建议框与真实框的重合程度来筛选
#-----重合程度大于0.5认为是正样本，反之为负样本，每次训练放入128个建议框训练，注意正负样本的平衡
