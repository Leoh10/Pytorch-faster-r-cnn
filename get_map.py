...
if __name__ == "__main__":
    '''
    Recall和Precision不像AP是一个面积的概念,因此在门限值-置信度(Confidence)不同时,网络的Recall和Precision值是不同的。
    ...
    #   map_mode为4代表利用COCO工具箱计算当前数据集的0.50:0.95map，计算IOU从0.5到0.95，步长变化为0.05，画P-R曲线
    # #需要获得预测结果、获得真实框后并安装pycocotools才行
    #-------------------------------------------------------------------------------------------------------------------#
    map_mode        = 0
    #--------------------------------------------------------------------------------------#
    ...
    #--------------------------------------------------------------------------------------#
    #   MINOVERLAP用于指定想要获得的mAP0.x，mAP0.x的意义是什么请同学们百度一下。mAP0.5即IOU为0.5
    #   比如计算mAP0.75，可以设定MINOVERLAP = 0.75。
    #
    #   当某一预测框与真实框重合度大于MINOVERLAP时，该预测框被认为是正样本，否则为负样本。IOU值设定，交并比
