...
class FRCNN(object):
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        #---------------------------------------------------------#
        #   添加上batch_size维度，preprocess_input是对图片进行归一化，除255像素值，
        #   transpose函数转换数据维度，将通道调整到第一维度，并利用expand_dims添加batch_size维度
        #   数据维度变为1*3*600*600
        #---------------------------------------------------------#
        image_data  = np.expand_dims(np.transpose(preprocess_input(np.array(image_data, dtype='float32')), (2, 0, 1)), 0)

        with torch.no_grad():
            images = torch.from_numpy(image_data)#将输入的图片数据从array转为tensor格式
            if self.cuda:#判断是否使用gpu
                images = images.cuda()
            ...
            #   rois          建议框的坐标
            #-------------------------------------------------------------#
            roi_cls_locs, roi_scores, rois, _ = self.net(images)#将图片传入网络中进行预测，得到建议框的位置，得分、坐标等
            #-------------------------------------------------------------#
            ...
            if len(results[0]) <= 0:
                return image
            # 对预测结果进行切分，第5序号的内容是获得的所有预测框的种类， 
            top_label   = np.array(results[0][:, 5], dtype = 'int32')
            #   第4个序号内容是所有预测框的置信度，前4个序号内容就是所有预测框的坐标
